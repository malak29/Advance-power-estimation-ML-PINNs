...
module_path = os.path.abspath(os.path.join('.','KUNAL_BF'))
...
from KUNAL_BF import Extended_Kalman_Filter_Smoother as EKFS
from KUNAL_BF import Unscented_Kalman_Filter_Smoother as UKFS
from KUNAL_BF import Gaussian_Filter_Smoother as GFS
from KUNAL_BF import Particle_Filter_Smoother as PFS
...
Q_d_PFS = np.reshape(np.array([[Q_model_PFS, 0, 0, 0],[0, Q_model_PFS, 0, 0],[0, 0, Q_params_PFS, 0],[0, 0, 0, Q_params_PFS]]), (4,4))
R_d_PFS = np.reshape(np.array([R_model_PFS]), (1,1))
...
x_model_nonlinear_filter_PFS = m_ini_model
...
for ii in range(y_sim_nonlinear_noisy.shape[1]):
    ...
    particles= SimplePedulum_Nonlinear_PFS.predict(u_k_model)

    # Particle Filter: Update Step
    weights= SimplePedulum_Nonlinear_PFS.update(y_sim_nonlinear_noisy[:, ii])

    # Storing the estimated states by calculating the weighted mean of particles
    x_k_filter = SimplePedulum_Nonlinear_PFS.weights
    x_model_nonlinear_filter_PFS= np.hstack((x_model_nonlinear_filter_PFS, x_k_filter))


# `x_model_nonlinear_filter_PFS` now contains the filtered state estimates over time
...
plt.subplot(311)
plt.plot(time_vector, x_sim_nonlinear_true[:,:-1].transpose(), linestyle='-', linewidth=3, label=[r'$\theta_{true}$ $(rads)$', r'$\omega_{true}$ $(rads/s)$'])
plt.plot(time_vector, x_model_nonlinear_filter_PFS[0:2,:-2].transpose(), linestyle='--', linewidth=1, label=[r'$\theta_{filter}$ $(rads)$', r'$\omega_{filter}$ $(rads/s)$'])
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('States '+ r'$(x)$', fontsize=12)
plt.title('Simple Pendulum Nonlinear System - True vs. Filtered States - PFS', fontsize=14)
plt.legend(loc='upper right')
plt.tight_layout()
plt.grid(True)
...
plt.subplot(312)
plt.plot(time_vector, g_true*np.ones((np.shape(time_vector)[0],1)), label=r'$g_{true}$ $(m/s^{2})$')
plt.plot(time_vector, x_model_nonlinear_filter_PFS[2,:-2].transpose(), linestyle='--', linewidth=1, label=r'$g_{filter}$ $(m/s^{2})$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(g)$', fontsize=12)
plt.title('Simple Pendulum Nonlinear System - True vs. Filtered Parameter ' + r'$g$', fontsize=14)
plt.legend(loc='upper right')
...
plt.subplot(313)
plt.plot(time_vector, L_true*np.ones((np.shape(time_vector)[0],1)), label=r'$L_{true}$ $(m)$')
plt.plot(time_vector, x_model_nonlinear_filter_PFS[3,:-2].transpose(), linestyle='--', linewidth=1, label=r'$L_{filter}$ $(m)$')
plt.xlabel('Time ' + r'$(sec)$', fontsize=12)
plt.ylabel('Parameter '+ r'$(L)$', fontsize=12)
plt.title('Simple Pendulum Nonlinear System - True vs. Filtered Parameter ' + r'$L$', fontsize=14)
plt.legend(loc='upper right')
