    ...
    for ii in range(Lag_Number):
        ...
        for jj in range(Total_Aggregation_Zone_Number):
            ...
            if ((ii == 0) and (jj ==0) ):
                ...
                Q_HVAC_A_Indices.append(4)

                Q_HVAC_B_Indices.append(4)
                ...
                Q_HVAC_A_Indices.append(Q_HVAC_A_Indices[-1]+6)

                Q_HVAC_B_Indices.append(Q_HVAC_B_Indices[-1]+5)
...
if (ANN_Type == 1): # MLP
    ...
    if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
        ...
        Current_ANNModel_A = tf.keras.Sequential([
            ...
            # layers.Dense(30, activation='relu'),
            # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
            layers.SimpleRNN((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
            #layers.Dense(7, activation='relu', kernel_regularizer='l2'),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
        ...
        Current_ANNModel_B = tf.keras.Sequential([
            ...
            # layers.Dense(30, activation='relu'),
            # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
            layers.SimpleRNN((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
            #layers.Dense(7, activation='relu', kernel_regularizer='l2'),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
    ...
    if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
        ...
        Current_ANNModel_A = tf.keras.Sequential([
            ...
            # layers.Dense(30, activation='relu'),
            # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
            layers.LSTM((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
        ...
        Current_ANNModel_B = tf.keras.Sequential([
            ...
            # layers.Dense(30, activation='relu'),
            # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
            layers.LSTM((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
    ...
    if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
        ...
        Current_ANNModel_A = tf.keras.Sequential([
            ...
            # layers.GRU(21, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
            # layers.GRU(14, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
            layers.GRU((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
        ...
        Current_ANNModel_B = tf.keras.Sequential([
            ...
            # layers.GRU(21, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
            # layers.GRU(14, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
            layers.GRU((6*Total_Aggregation_Zone_Number+1)*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
            layers.Dense(units=Total_Aggregation_Zone_Number, activation='linear')
        ])
            ...
            with tf.GradientTape() as tape:
                ...
                if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                    ...
                    if (ANN_Type == 1): # ANN
                        ...
                        x_batch_train_desired = [x_batch_train[:,i] for i in X_TF_A_Indices[-Total_Aggregation_Zone_Number:]]
                        x_batch_train_Tz_Prev = tf.cast(tf.reshape(x_batch_train_desired[0],[x_batch_train.shape[0],Total_Aggregation_Zone_Number]), tf.float32)
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                    ...
                    if (ANN_Type == 1): # ANN
                        ...
                        # Getting Tz_Previous
                        x_batch_train_Tz_Prev = tf.cast(tf.reshape(x_batch_train_A.numpy()[:,(Total_Aggregation_Zone_Number*(Lag_Number-1)):(Total_Aggregation_Zone_Number*Lag_Number)],[x_batch_train_A.shape[0],Total_Aggregation_Zone_Number]), tf.float32)
    ...
    if (ANN_Type == 1):
        ...
        for jj in range(Test_X_TF_Array_A.shape[0]):
            ...
            if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                ...
                # Test_Y_Predict[jj,:] = Test_X_TF_Array_A[jj,Tz_Indices[-Total_Aggregation_Zone_Number:]] + Current_ANNModel_A(Test_X_TF_Array_A[jj,:]) + Current_ANNModel_B(Test_X_TF_Array_B[jj,:])
        ...
        QHVAC_Indices = list(range(4, Test_X_TF_Array_B.shape[2]+1, 5))
        ...
        for jj in range(Test_X_TF_Array_A.shape[0]):
            ...
            # Test_X_TF_Array[jj, :, X_TF_A_Indices] = Test_X_TF_Array_A[jj, :, :]
            # Test_X_TF_Array[jj, :, X_TF_B_Indices] = Test_X_TF_Array_B[jj, :, :].transpose()
            ...
            Test_X_TF_Array_Reshaped_A = Test_X_TF_Array_Reshaped.numpy()[:,:,X_TF_A_Indices]
            Test_X_TF_Array_Reshaped_B = Test_X_TF_Array_Reshaped.numpy()[:,:,X_TF_B_Indices]           
            ...
            if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                ...
                Test_Y_Predict[jj,:] = tf.cast(tf.reshape(Test_X_TF_Array_Reshaped_A[0,Lag_Number-1,Tz_Indices],[1,Total_Aggregation_Zone_Number]), tf.float32) + Current_ANNModel_A(Test_X_TF_Array_Reshaped_A) + Current_ANNModel_B(Test_X_TF_Array_Reshaped_B)
...
if (NeuralStateSpace_ModelType == 1):
    ANNModel_FileName = ANNModel_Key + '_Dep1_' + str(Total_Aggregation_Zone_Number) + 'Zone' + kerasExtention
    Current_ANNModel.save(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_FileName))

elif (NeuralStateSpace_ModelType == 2):
    ANNModel_FileName_A = ANNModel_Key + '_Dep1_' + str(Total_Aggregation_Zone_Number) + 'Zone' + '_A' + kerasExtention
    Current_ANNModel_A.save(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_FileName_A))
    
    ANNModel_FileName_B = ANNModel_Key + '_Dep1_' + str(Total_Aggregation_Zone_Number) + 'Zone' + '_B' + kerasExtention
