...
for kk in range(Total_Aggregation_Zone_Number):
        ...
        if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
            ...
            for ii in range(Train_X_TF.shape[0]):
                ...
                if ((ii+(Batch_Size*Lag_Number)) <= Train_X_TF.shape[0]):
                    ...
                    Train_X_TF_batch_B = tf.reshape(Train_X_TF_batch_B, [Batch_Size, Lag_Number, Train_X_TF.shape[1]-1])
    ...
    if (ANN_Type == 1): # MLP
        ...
        if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
            ...
            Current_ANNModel_A = tf.keras.Sequential([
                ...
                # layers.Dense(30, activation='relu'),
                # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
                layers.SimpleRNN(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
                #layers.Dense(7, activation='relu', kernel_regularizer='l2'),
                layers.Dense(units=1, activation='linear')
            ])
            ...
            Current_ANNModel_B = tf.keras.Sequential([
                ...
                # layers.Dense(30, activation='relu'),
                # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
                layers.SimpleRNN(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
                #layers.Dense(7, activation='relu', kernel_regularizer='l2'),
                layers.Dense(units=1, activation='linear')
            ])
        ...
        if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
            ...
            Current_ANNModel_A = tf.keras.Sequential([
                ...
                # layers.Dense(30, activation='relu'),
                # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
                layers.LSTM(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
                layers.Dense(units=1, activation='linear')
            ])
            ...
            Current_ANNModel_B = tf.keras.Sequential([
                ...
                # layers.Dense(30, activation='relu'),
                # layers.Dense(30, activation='relu', kernel_regularizer='l2'),
                layers.LSTM(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
                layers.Dense(units=1, activation='linear')
            ])
        ...
        if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
            ...
            Current_ANNModel_A = tf.keras.Sequential([
                ...
                # layers.GRU(21, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
                # layers.GRU(14, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
                layers.GRU(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_A.shape[2]), return_sequences=False),
                layers.Dense(units=1, activation='linear')
            ])
            ...
            Current_ANNModel_B = tf.keras.Sequential([
                ...
                # layers.GRU(21, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
                # layers.GRU(14, input_shape=(1,Train_X_TF.shape[1]), return_sequences=True, kernel_regularizer='l2'),
                layers.GRU(7*Lag_Number, input_shape=(Lag_Number,Train_X_TF_11_B.shape[2]), return_sequences=False),
                layers.Dense(units=1, activation='linear')
            ])
            ...
            for ((x_batch_val_A, y_batch_val_A), (x_batch_val_B, y_batch_val_B)) in zip(val_dataset_A, val_dataset_B):                
                ...
                if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                    ...
                    if (ANN_Type == 1): # ANN
                        ...
                        # Getting Tz_Previous
                        x_batch_val_Tz_Prev = tf.cast(tf.reshape(x_batch_val_A[:,(Lag_Number*1)-1],[x_batch_val_A.shape[0],1]), tf.float32)
                        ...
                        # Getting Tz_Previous
                        x_batch_val_Tz_Prev = tf.cast(tf.reshape(x_batch_val_A[:,Lag_Number-1,0],[x_batch_val_A.shape[0],1]), tf.float32)
    ...
    if (NeuralStateSpace_ModelType == 1):  # Not Nueral State Space
        ...
        if (ANN_Type == 1):
            ...
            Test_Y_Predict = np.zeros((Test_X_TF_Array.shape[0],1))
            ...
            for jj in range(Test_X_TF_Array.shape[0]):
                ...
                if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                    ...
                    Test_Y_Predict[jj] = Test_X_TF_Array[jj,-1] + Current_ANNModel(np.reshape(Test_X_TF_Array[jj,:], (1, Test_X_TF_Array.shape[1])))
                ...
                PHVAC_Current[jj,0] = list(PHVAC_Dict.values())[0]

                # Feedback Step
                Tz_Current[0,:-1] = Tz_Current[0,1:]
                Tz_Current[0,-1] = Test_Y_Predict[jj]
            ...
            PHVAC1_Dict = PHVAC_Regression_Model(np.abs(np.reshape(np.array([Test_X['QAC_Corrected'].iloc[Lag_Number-1:]]), (Test_X['QAC_Corrected'].iloc[Lag_Number-1:].shape[0],1) )))
            ...
            # Initializing Initial Temp and Control Signals
            Tz_Current = np.reshape(Test_X_TF_Array[0,:,6], (1, Test_X_TF_Array.shape[1]))
        ...
        if (ANN_Type == 1):
            ...
            for jj in range(Test_X_TF_Array_A.shape[0]):
                ...
                if (TimeStepper_ModelType == 1): # Model Predicts Next State Directly
                    ...
                    Test_Y_Predict[jj] = Test_X_TF_Array_A[jj,-1] + Current_ANNModel_A(tf.reshape(Test_X_TF_Array_A[jj,:], (1, Test_X_TF_Array_A[jj,:].shape[0]))) + Current_ANNModel_B(tf.reshape(Test_X_TF_Array_B[jj,:], (1, Test_X_TF_Array_B[jj,:].shape[0])))
            ...
            for jj in range(Test_X_TF_Array_A.shape[0]):
                ...
                # Creating X for Current Timestep
                Test_X_TF_Array_B[jj,:,4] = QHVAC_Computed
                ...
                Test_X_TF_Array = np.concatenate((Test_X_TF_Array_B[jj,:,:],Test_X_TF_Array_A[jj,:,:]), axis=1)

                # Scaling of Input
                Test_X_Scaled = Scaler_Train_X.transform(Test_X_TF_Array.reshape([Lag_Number, Test_X_TF_Array.shape[1]]))

                # Reshaping Input for RNN
                Test_X_TF_Array_Reshaped = tf.reshape(Test_X_Scaled, [1,Lag_Number,Test_X_TF_Array.shape[1]])
                
                # Disaggregating Test_X_TF_Array_Reshaped into Test_X_TF_Array_Reshaped_A and Test_X_TF_Array_Reshaped_B
                Test_X_TF_Array_Reshaped_A = tf.reshape(Test_X_TF_Array_Reshaped[:,:,6], [Test_X_TF_Array_Reshaped.shape[0],Test_X_TF_Array_Reshaped.shape[1], 1])
                Test_X_TF_Array_Reshaped_B = tf.reshape(Test_X_TF_Array_Reshaped[:,:,0:6], [Test_X_TF_Array_Reshaped.shape[0],Test_X_TF_Array_Reshaped.shape[1], Test_X_TF_Array_B.shape[2]])
            ...
            Test_X_TF_reshaped_A = tf.reshape(Test_X_TF_reshaped[:,:,6], [Test_X_TF_reshaped.shape[0], Test_X_TF_reshaped.shape[1], 1])
    ...
    if (NeuralStateSpace_ModelType == 1):
        ANNModel_FileName = ANNModel_Key + '_Ind_' + str(Total_Aggregation_Zone_Number) + 'Zone_' + str(Aggregation_UnitNumber) + kerasExtention
        Current_ANNModel.save(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_FileName))

    elif (NeuralStateSpace_ModelType == 2):
        ANNModel_FileName_A = ANNModel_Key + '_Ind_' + str(Total_Aggregation_Zone_Number) + 'Zone_' + str(Aggregation_UnitNumber) + '_A' + kerasExtention
        Current_ANNModel_A.save(os.path.join(Sim_ANNModelData_FolderPath, ANNModel_FileName_A))
        
        ANNModel_FileName_B = ANNModel_Key + '_Ind_' + str(Total_Aggregation_Zone_Number) + 'Zone_' + str(Aggregation_UnitNumber) + '_B' + kerasExtention
