...
# This code has been modified to make it easy to skip simulations. 

# Created to resolve the issue with basement and slab objects. 
# The Simulation requires objecs created by the Basement and Slab programs. 
# These objects are defined in output files which are generated when the simulation is run. 
# When we use op.simulate, we put the output files in a different directory than the IDF and weather file.
# This makes it so that the simulation cannot complete, since it does not have the necessary definitions for the basement and slab objects. 

# One possible fix is: every single time we use op.simulate, put the IDF and Weather file in the temporary folder, and put the output files in the temprary folder. 
# Once the simulation is completely done, move the output files to where they need to go, and empty the temporary folder. 
...
SkipLocation = []
SkipHeatingType = []
SkipFoundationType = ['crawlspace', 'heatedbsmt', 'unheatedbsmt']
SkipSimulation = []
#SkipVariable = []
        ...
        for FileName in os.listdir(ClimateZone_SubfolderPath):
            if FileName.endswith('.idf'):
                 ...
                 if (SimulationType == 1):  # All required Files to be generated
                    ...
                    Initial_IDF_Run = op.simulate(Temporary_IDF_FilePath, Temporary_Weather_FilePath, base_dir_path = Temporary_FolderPath)
                         ...
                         if (os.path.isdir(os.path.join(Processed_BuildingSim_Data_FolderPath, SimulationName))):
                             ...
                             for OutputVariable_Name in Simulation_VariableNames:
                                 ...
                                 if skip_variable == 0:
                                     ...
                                     # Sim_OutputFiles_FolderPath
                                     op.simulate(Edited_IDFFile_Path, Edited_WeatherFile_Path, base_dir_path = Temporary_FolderPath)
                                     ...
                                     try:
                                         ...
                                         for file in os.listdir(Temporary_FolderPath):  
                                             filepath = os.path.join(Temporary_FolderPath, file)
                                             try:
                                                 if file.endswith('.idf') or file.endswith('.epw'):
                                                     os.remove(filepath)
                                                 else:
                                                     shutil.move(filepath, Sim_OutputFiles_FolderPath)
