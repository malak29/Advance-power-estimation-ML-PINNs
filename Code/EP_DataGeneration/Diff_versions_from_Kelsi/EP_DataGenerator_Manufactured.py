...
Manufactured_FolderPath = os.path.join(Current_FilePath, '..', '..', 'Data', 'Manufactured_Prototypes')

# Naming Convention: Configuration_Location_ClimateZone_CodeEdition_HeatingType
...
for Standard_Subfolder in os.listdir(Manufactured_FolderPath):
    Standard_SubfolderPath = os.path.join(Manufactured_FolderPath, Standard_Subfolder)
    # print(Year_SubfolderPath)

    for ClimateZone_Subfolder in os.listdir(Standard_SubfolderPath):
        ClimateZone_SubfolderPath = os.path.join(Standard_SubfolderPath, ClimateZone_Subfolder)
        ...
        for FileName in os.listdir(ClimateZone_SubfolderPath):
            if FileName.endswith('.idf'):
                 ...
                 IDF_FileName_Split = FileName.split('_')
                 Configuration = IDF_FileName_Split[0] # MS or SS
                 Location = IDF_FileName_Split[1] # Example: Miami
                 ClimateZone = IDF_FileName_Split[2] # Example: 1A
                 CodeEdition = IDF_FileName_Split[3] # Exapmple: Tier 1 
                 HeatingType = IDF_FileName_Split[4] # Example: elecricfurnace                
                     
                 SimulationName = Configuration + '_' + Location + '_' + ClimateZone + '_' + CodeEdition + '_' + HeatingType  
                 
                 print('Simulation Name: ' + SimulationName + '\n')
                 
                 # get path to weather file 
                 Weather_FolderPath = os.path.join(Manufactured_FolderPath, '..', 'TMY3_WeatherFiles_Manufactured')
                 ...
                 if Location in SkipLocation or HeatingType in SkipHeatingType or CodeEdition in SkipCodeEdition: 
