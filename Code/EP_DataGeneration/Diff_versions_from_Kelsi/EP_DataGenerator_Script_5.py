...
#Created to add continegency to skip simulation if it's already been completed.
...
'''
IDF_Type = "ASHRAE"
#IDF_Types_List = ["ASHRAE", "IECC"]

#For Testing Purposes
IDF_Types_List = ["ASHRAE"]
...
'''
IDF_Year = "2013"
#ASHRAE_Years_List = ["2013", "2016", "2019"]
#IECC_Years_List = ["2012", "2015", "2018"]

#For Testing Purposes
ASHRAE_Years_List = ["2013"]
IECC_Years_List = ["2012"]
...
- WareHouse
'''
IDF_BuildingType = "Hospital"
IDF_BuildingTypes_List =[
    "OfficeSmall", 
    "OfficeMedium", 
    #"OfficeLarge", #new error
    "ApartmentHighRise", 
    "Hospital",
    "HotelLarge",
    "HotelSmall",
...
#For Testing Purposes
#IDF_Locations_List = ["Albuquerque"]
...
Total_Simulation = len(IDF_Types_List)*(len(ASHRAE_Years_List)+len(IECC_Years_List))*len(IDF_Locations_List)*len(IDF_BuildingTypes_List)

#Estimated Simulation Time: No CSV's
#Average Simulation Time 4 minutes per simulation.
#Total_Simulation_Time = Total_Simulation * 4

#Estimated Simulation: With CSV's
#Total_Simulation_Time = Total_Simulation * 

for IDF_Type in IDF_Types_List:

    #If else for selecting year list.
    if (IDF_Type == "ASHRAE"):
            
            Years_List = ASHRAE_Years_List

    elif (IDF_Type == "IECC"):

            Years_List = IECC_Years_List 

    for IDF_Year in Years_List:

        for IDF_Location in IDF_Locations_List:

            for IDF_BuildingType in IDF_BuildingTypes_List:
                ...
                Simulation_Name = "".join([IDF_Type, "_", IDF_Year, "_", IDF_Location, "_", IDF_BuildingType])

                print("\n")
                print("Simulation Name: " + Simulation_Name)
                print("\n")
                print("Progress: " + str(Simulation_Counter) + "/" + str(Total_Simulation))
                print("/n")

                #Without CSV's 
                #print("Estimated Time Remaining: " + str(4 * (Total_Simulation - Simulation_Counter)) + " Minutes")
                ...
                for FileName in Weather_FileName_List:
                    ...
                    if (FileName.find(IDF_Location_2) != -1):
                ...
                NotEmpty = os.path.exists(EIO_FilePath)
                ...
                # the code must think that the folders exist when they acutally don't
                # it's checking whether outer folder exists, not whether inner folders exist
                # confirmed - if I delete the empty folders, the code works
                ...
                #add when commenting out csv code
                ...
                for OutputVariable_Name in Simulation_VariableNames:
                    ...
                    print("CSV: ", OutputVariable_Name)
                    print("\n")
                    csv_counter += 1
                    print("CSV's: ", csv_counter, "/35")
                ...
                # Don't make this pickle file - it's not used
