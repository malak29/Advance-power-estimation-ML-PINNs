...
ANN_Type = 1  # 1 - ANN , 2 - Simple RNN , 3 - LSTM , 4 - GRU
...
if (ANN_Type == 1):
    ...
    for jj in range(Test_X_TF_Array.shape[0]):
        ...
        input_sample = np.expand_dims(Test_X_TF_Array[jj,:], axis=0)  # Shape (1, 13)
        predicted_output = Current_ANNModel(input_sample)
        Test_Y_Predict[jj,:] = predicted_output[0]
        ...
        for kk in range(Total_Aggregation_Zone_Number):
            ...
            predicted_PHVAC = PHVAC_Regression_Model(QHVAC_Computed_Array)
            # Extract tensor from dictionary and convert to float
            tensor_value = predicted_PHVAC['output_0'].numpy()[0,0]
            # PHVAC_Current[jj, kk] = float(tensor_value[0, 0])
            PHVAC_Current[jj,kk] = tensor_value
