...
for kk in range(Total_Aggregation_Zone_Number):
    ...
    if (ANN_Type == 1):
        ...
        PHVAC1_Current = PHVAC_Regression_Model.predict(Test_X['QAC_Corrected'].abs())
    ...
    Train_PercentageAccuracy = np.mean(((np.absolute(Train_Y_Predict1-np.reshape(Train_Y.to_numpy(),(Train_Y_Predict1.shape[0],1))))/(np.reshape(Train_Y.to_numpy(),(Train_Y_Predict1.shape[0],1)))) *100)
    ...
    Predict_Actual_Y_Dict = {'DateTime': DateTime_Test.tolist()[1:], 'Predict_Y': Predict_Y_List, 'Actual_Y': Actual_Y_List}
    ...
    Predict_Actual_Y_Dict1 = {'DateTime': DateTime_Test.tolist()[1:], 'Predict_Y': Predict_Y_List1, 'Actual_Y': Actual_Y_List1}
...
Predict_Actual_PHVAC1_Dict = {'DateTime': DateTime_Test.tolist()[:-1], 'Predict_Y': Predict_Y_List, 'Actual_Y': Actual_Y_List}
...
Predict_Actual_PHVAC_Dict = {'DateTime': DateTime_Test.tolist()[:-1], 'Predict_Y': Predict_Y_List, 'Actual_Y': Actual_Y_List}
